...
setYMD(date,options.year,options.month,options.date);function changeView(v){if(v!=viewName){ignoreWindowResize++;viewUnselect();var oldView=view,newViewElement;if(oldView){if(oldView.eventsChanged){eventsDirty();oldView.eventDirty=oldView.eventsChanged=false;}
...
html+="<div class='"+className+event.className.join(' ')+"' style='position:absolute;left:"+left+"px'"+" rel='"+event.id+"'>"+"<a"+(event.url?" href='"+htmlEscape(event.url)+"'":'')+">"+
...
((event.editable||event.editable===undefined&&options.editable)&&!options.disableResizing&&$.fn.resizable?"<div class='ui-resizable-handle ui-resizable-"+(rtl?'w':'e')+"'></div>":'')+"<div class='fc-event-description'><span class='bubble'><span class='time'>"+getEventTimeString(event.start)+"</span>"+htmlEscape(event.description)+"</span></div>"+"</div>";seg.left=left;seg.outerWidth=right-left;}
